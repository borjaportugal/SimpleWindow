...
namespace ImGui
{
	...
	// Window
	IMGUI_API bool          Begin(const char* name, bool* p_opened = NULL, ImGuiWindowFlags flags =
								  ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_ShowBorders);                                           // see .cpp for details. return false when window is collapsed, so you can early out in your code. 'bool* p_opened' creates a widget on the upper-right to close the window (which sets your bool to false).
	...
	// HACK: changed the name of the function for not colliding with windows API
	IMGUI_API ImFont*       GetWindowFont_();
