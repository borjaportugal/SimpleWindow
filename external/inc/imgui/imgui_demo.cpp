...
void ImGui::ShowTestWindow(bool* opened)
{
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        if (ImGui::TreeNode("Fonts", "Fonts (%d)", ImGui::GetIO().Fonts->Fonts.Size))
        {
            ...
            for (int i = 0; i < atlas->Fonts.Size; i++)
            {
                ...
                ImGui::TreePush((void*)(std::size_t)i);
    ...
    if (ImGui::CollapsingHeader("Widgets"))
    {
        if (ImGui::TreeNode("Tree"))
        {
            for (int i = 0; i < 5; i++)
            {
                if (ImGui::TreeNode((void*)(std::size_t)i, "Child %d", i))
                {
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        if (ImGui::TreeNode("Scrolling"))
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                ImGui::BeginChild(ImGui::GetID((void*)(std::size_t)i), ImVec2(ImGui::GetWindowWidth() * 0.17f, 200.0f), true);
                if (scroll_to)
        ...
        if (ImGui::TreeNode("Clipping"))
        {
            ...
            ImGui::GetWindowDrawList()->AddText(ImGui::GetWindowFont_(), ImGui::GetWindowFontSize()*2.0f, ImVec2(pos.x+offset.x,pos.y+offset.y), ImColor(255,255,255,255), "Line 1 hello\nLine 2 clip me!", NULL, 0.0f, &clip_rect);
